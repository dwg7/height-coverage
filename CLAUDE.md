...
`tunnel.optgeo.org`（taroverture）は個人運用のCloudflare Tunnel経由の開発用エンドポイントであり、本番CDNのような安定性は保証されない。動作検証中、特定のタイル（パリ中心部の高密度タイルなど）に対してOriginヘッダ付きリクエストが継続的にCloudflareの530エラーを返す一方、同一タイルへのOriginヘッダ無しリクエスト（curl等）は200を返す、という非対称な挙動を確認した。ヴィエンチャンAOIのタイルは安定して200を返した。これはこのアプリのコードの不具合ではなく、上流の個人インフラの既知の脆弱性として受け入れる（[DECISIONS.md](DECISIONS.md)参照）。将来的に本番運用する場合は、taroverture側のホスティングをより安定した環境に移すことが望ましい。
