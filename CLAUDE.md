...
- 地図上のどこをクリックしても（建物の上である必要はない）、クリック地点を中心に8方向のコンパスUIが開く。Street Viewは道路から見るものだが、OSMの道路網も建物と同様に完全ではないため、道路上限定にはしていない。各ボタンはGoogle Street Viewへのリンクで、画面上の見た目の方向を`map.getBearing()`で実世界の方位に変換した`heading`パラメータを付与する（地図を回転させても正しい方向を指す）。黄色（未入力）の建物の上でクリックした場合のみ、コンパス中央に鉛筆アイコンの編集ボタン（iDエディタへのリンク）が追加で出る——階数入力は未入力の建物にしか意味がないため。初回クリック時のみ、Street Viewが編集の下敷きに使うべきソースではない旨の注記ダイアログ（localStorage記録・以後表示しない）が挟まる。姉妹プロジェクト`dwg7/vientiane-planning-map`が先に実装・デバッグ済みだったコンパスUIをほぼそのまま移植したもの（中央ボタンと、クリック範囲を建物に限定しない点はこちら独自の変更。経緯は[DECISIONS.md](DECISIONS.md)項目13・14参照）。
